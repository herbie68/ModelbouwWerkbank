...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `view_projectcosts` AS select `prj`.`Name` AS `ProjectName`,`cat`.`Id` AS `Category`,`prd`.`Name` AS `ProductName`,sum(`pu`.`AmountUsed`) AS `AmountUsed`,`prd`.`Price` AS `Price`,(sum(`pu`.`AmountUsed`) * `prd`.`Price`) AS `Total` from (((`productusage` `pu` join `project` `prj` on((`pu`.`project_Id` = `prj`.`Id`))) join `product` `prd` on((`pu`.`product_Id` = `prd`.`Id`))) join `category` `cat` on((`prd`.`Category_Id` = `cat`.`Id`))) group by `prd`.`Name` order by `prj`.`Name`,`cat`.`Id`,`prd`.`Name`
;
