...
CREATE ALGORITHM=MERGE SQL SECURITY DEFINER VIEW `view_stock` AS select `s`.`Product_Id` AS `product_Id`,`p`.`Name` AS `ProductName`,`p`.`Storage_Id` AS `Storage_Id`,`st`.`Name` AS `StorageName`,`s`.`Amount` AS `Amount`,`p`.`Unit_Id` AS `Unit_Id`,`u`.`Name` AS `UnitName` from (((`view_instock` `s` join `product` `p` on((`s`.`Product_Id` = `p`.`Id`))) join `storage` `st` on((`p`.`Storage_Id` = `st`.`Id`))) join `unit` `u` on((`p`.`Unit_Id` = `u`.`Id`)))
;
